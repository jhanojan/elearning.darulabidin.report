                        ...
                        $child=GetAll('sv_parent_child',array('parent'=>'where/'.$val['id']));
                            if($child->num_rows()>0){
                                foreach($child->result_array() as $anaks){
                                    $detailanak=$this->mdlfo->query("SELECT firstname,lastname FROM mdlfo_user WHERE id='$anaks'")->row_array();
                        ?>
                            <div class="row">
...
<script>
    ...
    function hapusanak(ids){
        i = confirm('Hapus : ' + ids + ' ?');
        if (i) {
            window.location = '<?php echo base_url()?>master_orang_tua/delete/' + ids;
        }
    }
</script>