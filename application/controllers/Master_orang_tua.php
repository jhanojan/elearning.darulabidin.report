...
class Master_orang_tua extends CI_Controller {
	...
	function submit(){ 
        ...
        function searchsis(){
            ...
            $ambilsiswa=$this->mdlfo->query("SELECT id,firstname,lastname FROM mdlfo_user WHERE lastname LIKE '%$cari%' ORDER BY lastname ASC")->result();
            foreach($ambilsiswa as $aw){
                $data['results'][]=array(
                    'id'=>$aw->id,
                    'text'=>$aw->lastname.' ('.$aw->firstname.')'
                );
            }
